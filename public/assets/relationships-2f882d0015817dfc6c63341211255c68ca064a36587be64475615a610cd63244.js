(function() {
  ...
  $(document).ready(function() {
    ...
    $('.relationship-block').on('click', '.unfollow', function() {
      return editRelationship(id, 'DELETE', API.getRelationshipsUrl().format(id), DEC_FOLLOWERS);
    });
    return $('.relationship-block').on('click', '.follow', function() {
      return editRelationship(id, 'POST', API.getRelationshipsUrl(true).format(id), INC_FOLLOWERS);
    });
  });
